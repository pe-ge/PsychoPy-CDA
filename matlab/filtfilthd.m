function y = filtfilthd(Hd,x)
%FILTFILT Zero-phase forward and reverse digital filtering.
%   Y = FILTFILT(Hd, X) filters the data in vector X with the filter described
%   by Hd to create the filtered data Y.  The filter is described 
...
%   01/07/09 Modified by LJT to allow filter structure input (Hd)

%    error(nargchk(2,2,nargin))
    if (isempty(Hd) || isempty(x))
        y = [];
        return
    end
...
% LJT: Here I convert the filter specified by Hd to Direct Form II
% transposed so I can count the single-section equivalent number of
% coefficients
    df2t=convert(Hd,'df2t');
    b = df2t.Numerator;
    a = df2t.Denominator;
    nb = length(b);
    na = length(a);
